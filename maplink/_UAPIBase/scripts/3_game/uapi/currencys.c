...
class UCurrency extends UCurrencyBase
{
	...
	{
		UCurrency uCurrency = new UCurrency;
		uCurrency.Create(currency);
		m_UCurrencysMap.Set(key, uCurrency);
		return uCurrency;
	}
	
	static UCurrency InsertNew(string key, array<ref UCurrencyValue> values)
	{
		UCurrency uCurrency = new UCurrency;
		for (int i = 0; i < values.Count(); i++)
		{
			UCurrencyValue value = UCurrencyValue.Cast(values.Get(i));
			UCurrencyValue v = new UCurrencyValue(value.TypeClass(), value.Value());
			uCurrency.Insert( v );
		}
	...
	void SortCurrency()
	{
		...
		array<ref UCurrencyValue> StartingValues =  new array<ref UCurrencyValue>;
		array<ref UCurrencyValue> SortedMoneyValues = new array<ref UCurrencyValue>;
