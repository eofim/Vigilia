class UApiEntityStore extends UApiObject_Base
{
	...
	protected ref array<ref UApiZoneHealthData> m_HealthZones;
	protected ref array<ref UApiEntityStore> m_Cargo;

	bool m_IsMagazine;
	ref array<ref UApiAmmoData> m_MagAmmo;
	bool m_IsWeapon;
	bool m_IsVehicle;
	ref array<int> m_FireModes;
	ref UApiAmmoData m_ChamberedRound;

	protected ref array<ref UApiMetaData> m_MetaData;
	...
	bool SaveZoneHealth(string zone, float health)
	{
		if (!m_HealthZones)
		{
			m_HealthZones = new array<ref UApiZoneHealthData>;
		}
