class PlayerDataStore extends Managed
{
	...
	int m_BleedingBits;
	ref TIntArray m_BleedingActiveTime;
	int	m_LifeSpanState;
	...
	ref array<ref UApiPlayerZoneHealthData> m_HealthZones;
	ref array<ref UApiPlayerIdFloatData> m_Modifiers;
	ref array<ref UApiPlayerIdFloatData> m_Agents;
	ref array<ref UApiMetaData> m_Stats;
	ref array<ref UApiEntityStore> m_Attachments;
	ref array<ref UApiStomachItem> m_Stomach;
	ref array<ref UApiMetaData> m_MetaData;
	...
	void AddModifier(int id, float attachedTime) 
	{
		if (!m_Modifiers)
		{
			m_Modifiers = new array<ref UApiPlayerIdFloatData>;
		}
	...
	void AddAgent(int key, float value) 
	{
		if (!m_Agents)
		{
			m_Agents = new array<ref UApiPlayerIdFloatData>;
		}
	...
	void AddStomachItem(float amount, int foodstage, string className, int agents)
	{
		if (!m_Stomach)
		{ 
			m_Stomach = new array<ref UApiStomachItem>; 
		}
	...
	bool AddStat(string label, float data)
	{
		if (!m_Stats) 
		{ 
			m_Stats = new array<ref UApiMetaData>;
		}
	...
	bool Write(string var, bool data)
	{
		if (!m_MetaData) 
		{ 
			m_MetaData = new array<ref UApiMetaData>;
		}
	...
	int GetInt(string var)
	{
		for(int i = 0; i < m_MetaData.Count(); i++)
		{
			if (m_MetaData.Get(i) && m_MetaData.Get(i).Is(var))
			{
				return m_MetaData.Get(i).ReadInt();
			}
		}
	...
	float GetFloat(string var)
	{
		for(int i = 0; i < m_MetaData.Count(); i++)
		{
			if (m_MetaData.Get(i) && m_MetaData.Get(i).Is(var))
			{ 
				return m_MetaData.Get(i).ReadFloat(); 
			}
		}
	...
	vector GetVector(string var)
	{
		for(int i = 0; i < m_MetaData.Count(); i++)
		{
			if (m_MetaData.Get(i) && m_MetaData.Get(i).Is(var))
			{ 
				return m_MetaData.Get(i).ReadVector(); 
			}
		}
	...
	string GetString(string var)
	{
		for (int i = 0; i < m_MetaData.Count(); i++)
		{
			if (m_MetaData.Get(i) && m_MetaData.Get(i).Is(var))
			{ 
				return m_MetaData.Get(i).ReadString(); 
			}
		}
	...
	bool SaveZoneHealth(string zone, float health, float blood, float shock)
	{
		if (!m_HealthZones)
		{
			m_HealthZones = new array<ref UApiPlayerZoneHealthData>
		}
